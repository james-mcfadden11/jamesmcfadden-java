<template>
  <table id="tblUsers">
    ...
    <tbody>
      ...
      <tr v-bind:class="{ disabled: user.status == 'Disabled' }" v-for="user in filteredList" v-bind:key="user.emailAddress">
    ...
    filteredList() {
      let filteredUsers = this.users.filter(user => {
        return (user.firstName.toLowerCase().includes(this.filter.firstName.toLowerCase()) 
              ...
              && (user.status == this.filter.status || this.filter.status == ""))
      })
