...
export default {
  ...
  methods: {
    ...
    deleteCard() {
      const isConfirmed = confirm("Are you sure you want to delete this card? This action cannot be undone.");
      if (isConfirmed) {
        boardsService
          ...
          .catch(error => {
            if (error.response) {
              this.errorMsg = `Error deleting card. Response received was ' ${error.response.statusText}'.`;                "'.";
