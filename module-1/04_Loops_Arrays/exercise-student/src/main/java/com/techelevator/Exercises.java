...
public class Exercises {
	...
	public int sum13(int[] nums) {
		boolean wasPreviousNumberThirteen = false;
		int sum = 0;
		for (int i = 0; i < nums.length; i++) {
			if (nums[i] == 13) {
				// dont count it, toggle to true
				wasPreviousNumberThirteen = true;
			} else if (wasPreviousNumberThirteen) {
				// dont count it, toggle back to false
				wasPreviousNumberThirteen = false;
			} else {
				// count it
				sum += nums[i];
			}
